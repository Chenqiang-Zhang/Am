// Local Neo4j import script for Amazon Reviews'23 All_Beauty KG.
// Copy kg_output/all_beauty/ to $NEO4J_HOME/import/all_beauty/ before running.
// For Neo4j Aura, use neo4j/all_beauty_import_github_chunks.cypher instead.
...
LOAD CSV WITH HEADERS FROM 'file:///all_beauty/nodes_users.csv' AS row
MERGE (:User {user_id: row.user_id});


LOAD CSV WITH HEADERS FROM 'file:///all_beauty/nodes_products.csv' AS row
...
LOAD CSV WITH HEADERS FROM 'file:///all_beauty/nodes_reviews.csv' AS row
...
LOAD CSV WITH HEADERS FROM 'file:///all_beauty/nodes_categories.csv' AS row
MERGE (c:Category {category_id: row.category_id})
SET c.name = row.name;


LOAD CSV WITH HEADERS FROM 'file:///all_beauty/nodes_stores.csv' AS row
MERGE (s:Store {store_id: row.store_id})
SET s.name = row.name;


LOAD CSV WITH HEADERS FROM 'file:///all_beauty/nodes_features.csv' AS row
MERGE (f:Feature {feature_id: row.feature_id})
SET f.text = row.text,
    f.normalized_text = row.normalized_text;

LOAD CSV WITH HEADERS FROM 'file:///all_beauty/rel_wrote.csv' AS row
MATCH (u:User {user_id: row.user_id})
MATCH (r:Review {review_id: row.review_id})
MERGE (u)-[:WROTE]->(r);


LOAD CSV WITH HEADERS FROM 'file:///all_beauty/rel_reviews.csv' AS row
MATCH (r:Review {review_id: row.review_id})
MATCH (p:Product {product_id: row.product_id})
MERGE (r)-[:REVIEWS]->(p);


LOAD CSV WITH HEADERS FROM 'file:///all_beauty/rel_rated.csv' AS row
MATCH (u:User {user_id: row.user_id})
MATCH (p:Product {product_id: row.product_id})
MERGE (u)-[rel:RATED]->(p)
...
LOAD CSV WITH HEADERS FROM 'file:///all_beauty/rel_product_category.csv' AS row
MATCH (p:Product {product_id: row.product_id})
MATCH (c:Category {category_id: row.category_id})
MERGE (p)-[:BELONGS_TO]->(c);


LOAD CSV WITH HEADERS FROM 'file:///all_beauty/rel_product_store.csv' AS row
MATCH (p:Product {product_id: row.product_id})
MATCH (s:Store {store_id: row.store_id})
MERGE (p)-[:SOLD_BY]->(s);


LOAD CSV WITH HEADERS FROM 'file:///all_beauty/rel_product_feature.csv' AS row
MATCH (p:Product {product_id: row.product_id})
