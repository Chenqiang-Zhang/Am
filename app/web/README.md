...
React + Vite + TypeScript。バックエンド（`../api`）の `POST /chat` を中心に使い、
会話コンテキストに基づいて商品を推薦する。推薦結果は「なぜ推薦されたか」の根拠
（Text2Cypherが生成したCypherの一文説明・一致した構造化属性）とユーザーフィードバック付きで表示する。
...
dev では `/api` を `:8000` へ proxy する（`vite.config.ts`）。CORS 設定は不要。
...
├── pages/ChatPage.tsx   # 会話状態・API 呼び出しを集約（唯一のページ。App.tsx から表示）
...
3. `components/` に検索状態を持ち込まない。会話状態は `ChatPage` に集約。
4. v1 では状態管理ライブラリを入れない（`useState` のみ）。スタイルは CSS Modules。

## 実装済み要件

- 商品情報の多言語表示フィールド
- 会話コンテキストに基づくレコメンド（Text2Cypherへ委譲）
- レコメンド理由のフィードバック
- デモ用テストユーザー選択（匿名 / オリジナルテストユーザー / 実ユーザー）
